...
.provider('ShoppingListCheckOffServiceProvider',ShoppingListCheckOffServiceProvider)
.config(Config);

Config.$inject = ['ShoppingListCheckOffService'];
function Config(ShoppingListCheckOffService){

  ShoppingListCheckOffServiceProvider.itemsToBuy=[{
                name:"apple",
                quantity:"1"
              },
              {
...
//handles bought items - removes from to buy and moves to bought
  ...
  provider.$get = function(){
    var itemsToBuy = new ShoppingListCheckOffServiceProvider();
    return itemsToBuy;
